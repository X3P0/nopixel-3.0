...
RegisterNetEvent('np-infinity:hotzones:updateList')
AddEventHandler('np-infinity:hotzones:updateList', function(zones)
...
Citizen.CreateThread(function()
    ...
    TriggerServerEvent('np-infinity:hotzones:requestList')

    while GetResourceState('np-density') ~= 'started' do
        Citizen.Wait(100)
    end

    exports['np-density']:RegisterDensityReason('hotzone', 2)
        ...
        for zoneId, zone in pairs(HotZones) do
            ...
            if isNear and not zone.inside and zone.tick >= 2 then
                ...
                if SpawnControl then
                    exports['np-density']:ChangeDensity('hotzone', 0.1)
                end
                TriggerEvent('np-voice:setQuietMode', 'interior', true, 2)
                TriggerServerEvent('np-infinity:hotzones:enteredZone', zoneId)
            ...
            elseif not isNear and zone and zone.inside and zone.tick <= 1 then
                zone.inside = false
                if SpawnControl then
                    exports['np-density']:ChangeDensity('hotzone', -1)
                end
                TriggerEvent('np-voice:setQuietMode', 'interior', false)
                TriggerServerEvent('np-infinity:hotzones:exitZone', zoneId)
