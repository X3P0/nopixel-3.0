		...
		if victim ~= nil and attacker ~= nil then
			if victimDied then
				...
				if not IsEntityAVehicle(victim) then
					--For some reason if a player kills a player, the variable victimDied isn't returned as true.
					--Therefore we need to do a check here.
					if IsEntityAPed(victim) and IsEntityAPed(attacker) then
						if IsPedAPlayer(victim) and IsPedAPlayer(attacker) then
							...
							if IsPlayerDead(victimPlayer) then
								local attackerPlayer = NetworkGetPlayerIndexFromPed(attacker)

								PlayerKilledByPlayer(victimPlayer, attackerPlayer, weapon, isMelee)
							end
						end
					end
...
function PlayerKilledByPlayer(victim, attacker, weapon, isMelee)
	TriggerEvent('DamageEvents:PlayerKilledByPlayer', victim, attacker, weapon, isMelee)
end
