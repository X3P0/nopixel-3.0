...
local TalkingCache = CacheableMap(function (ctx, pPed)
    if not pPed then return false, nil end

    local isTalking = DecorGetBool(pPed, 'isTalking')

    return true, isTalking
end, { timeToLive = 250 })

function IsTalking(pPed)
    return TalkingCache.get(pPed)
end

function displayPlayerNumbers(isDisplaying)

    if not isDisplaying then playerNumber = false return end
    if playerNumber then return end
    if isDisplaying then playerNumber = true end

    Citizen.CreateThread(function()
        Wait(1600)
        while playerNumber do
            for i=0,255 do
            ...
            for id = 0, 255 do
                if NetworkIsPlayerActive(id) then
                    ...
                    if ped == playerped then
                        ...
                        if isDucking or isStealth == 1 or isDriveBy or isInCover or not isVisible then
                            cansee = false
                        end
