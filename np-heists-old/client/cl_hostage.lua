...
local function grab(entity)
    ...
    Wait(2000)
    exports["np-ui"]:showInteraction("[F] Release!", "success")
...
local function showInteraction()
    if not interactionUp then
        ...
        exports["np-ui"]:showInteraction("[G] Grab!", "error")
    end
end
...
AddEventHandler("np-casino:casinoEnteredEvent", function() inCasino = true end)
AddEventHandler("np-casino:casinoExitedEvent", function() inCasino = false end)

AddEventHandler('np-heist:disableHostage', function(pDisabled)
    hostageDisabled = pDisabled
end)
    ...
    while true do
        ...
        then
            ...
            if lastEntity ~= entity then
                local flags = exports["np-flags"]:GetPedFlags(entity)
