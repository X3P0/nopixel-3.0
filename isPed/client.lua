...
function isPed(checkType)
    ...
    if checkType == "phone_number" then
      pass = phone_number
    end
    ...
    if checkType == "recentcuff" then
      if GetGameTimer() - lastTimeCuffed > 15000 then 
        pass = false
      else
        pass = true
      end
...
RegisterNetEvent("isPedUpdate")
AddEventHandler("isPedUpdate", function(firstname, lastname, phonenumber, pCid)
    Firstname = firstname
    Lastname = lastname
...
AddEventHandler('onResourceStart', function(resource)
    if ResourceName == resource or not cid or cid <= 0 then return end

    TriggerEvent(('%s:hotreload'):format(resource), cid)
end)
