    ...
    while isInstructorMode do
      ...
      if isInVehicle then
        ...
        if playerVeh ~= 0 and isVehicleAllowed(playerVeh) then -- Check if we weren't in a vehicle before but now are
          ...
          if exports['np-vehicles']:HasVehicleKey(playerVeh) then
            isInVehicle = true
...
AddEventHandler("np-jobmanager:playerBecameJob", function(job, name, notify)
  if (job == "driving instructor") then
    TriggerServerEvent('driving:toggleInstructorMode', true)
  elseif myJob == "driving instructor" then
    TriggerServerEvent('driving:toggleInstructorMode', false)
  end
  myJob = job
end)
