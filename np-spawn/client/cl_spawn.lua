...
function Login.ShowMenu(toggle)
	...
	local lifersDisabled = exports["np-config"]:GetMiscConfig("characters.lifers.disabled")
...
function Login.nativeStart(AlreadySpawned)
	...
	if not AlreadySpawned then
		TriggerEvent('housing:getCurrentList')
		...
		Login.Init()
		GetCharacterData.reset()
		GetClothingData.reset()
		Login.CreatePeds(false)
...
RegisterNUICallback("nuiCallback", function(data, callback)
	...
	if data.action == "newCharacter" then
		...
		local chardata = {
			...
			dob = cData.dob,
			gender = gender,
			type = cData.type or "normal",
			story = "Default story - new char system"
		}
		
        if not chardata then callback({}) return end

		if cData.type and cData.type == 'hardcore' then
			local canCreate = RPC.execute('np-spawn:canCreateHardcoreCharacter')
			if not canCreate then callback({}) return end
		end

		local result = NPX.Procedures.execute('np-character:createCharacter', chardata)
		...
		if not result.success then
			TriggerEvent("DoLongHudText", result.message, 2)
		end
		Login.actionsBlocked = false
