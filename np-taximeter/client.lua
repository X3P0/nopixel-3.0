...
AddEventHandler('taximeter:PerMinute', function(numsent)
  ...
  if IsPedInAnyVehicle(PlayerPedId(), false) then
    if (GetEntityModel(GetVehiclePedIsUsing(PlayerPedId())) == `taxi` or GetEntityModel(GetVehiclePedIsUsing(PlayerPedId())) == `dilettante3`) and playerPed == driverPed then
      PerMinute = numsent
      Total = BaseFare
      --updateDriverMeter()
...
AddEventHandler('taximeter:FreezePlate', function(platesent,result)
  ...
  if plate == platesent and (GetEntityModel(GetVehiclePedIsUsing(PlayerPedId())) == `taxi` or GetEntityModel(GetVehiclePedIsUsing(PlayerPedId())) == `dilettante3`) and playerPed ~= driverPed then
    frozen = result
  end
end)
