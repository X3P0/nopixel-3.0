window.onload = function(e)
{
    ...
    function toggleMenu(state, menu)
    {
        if(state)
        {
            if(menuStructure[menu].selectedItem < maxMenuItems)
            {
                ...
                $.post("https://np-bennys/selectedItem", JSON.stringify({
                    id: val1,
                    item: val2,
                    item2: val3
                }));
    ...
    function updateItem2TextOnly(menu, id, text)
    {
        ...
        $.post("https://np-bennys/updateItem2", JSON.stringify({
            item: text
        }));
    }
    ...
    function updateItem2Text(menu, id, text)
    {
        ...
        else if(id != menuStructure[menu].previousSelectedItemID)
        {
            ...
            let menuItem = menuStructure[menu].itemsArray[prevID + 1];
            if (menuItem)
                menuItem.getElementsByClassName("item2")[0].textContent = menuStructure[menu].items[prevID].item2;
            menuStructure[menu].previousSelectedItemID = id;
    ...
    function setTaxLevel(pTaxLevel) {
        taxLevel = pTaxLevel;
    }

    function updateBoughtParts(partsList) {
        let totalPrice = 0;
        let partsHtml = "";
        for (let part of partsList) {
            totalPrice += part.price;
            partsHtml += `<li>${part.name} - <span class="money">$${part.price}</span></li>`;
        }
        const taxPrice = Math.ceil(totalPrice * (1 + taxLevel / 100));
        $('#pricelist').fadeIn();
        const priceWithTaxString = `$${taxPrice.toFixed(2)} Incl. ${taxLevel}% tax`;
        $('#totalcost').html(priceWithTaxString);
    ...
    window.addEventListener("message", function (event) {
        ...
        if (eventData.updateBoughtParts) {
            updateBoughtParts(eventData.updateBoughtParts);
        }

        if (eventData.setTaxLevel) {
            setTaxLevel(eventData.setTaxLevel);
        }
    });
}
