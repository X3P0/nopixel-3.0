...
on('np-inventory:openDumpster', (pParams, pEntity, pContext) => {
    openDumpster(pEntity);
});

on('np-inventory:openTrunk', (pParams, pEntity, pContext) => {
    openTrunk(pEntity);
});
...
function hasPermissionInProperty(){
    if(exports["np-housing"].isInsideProperty() && exports["np-housing"].hasPermissionInProperty() == true && exports["np-housing"].isPropertyShop())
        return true
    else
        return false 
}
