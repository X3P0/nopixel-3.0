...
function NPC:setSetting(mode, active, settings)
    ...
    elseif mode == "dead" and active then
        ApplyDamageToPed(self.entity, 9999.9, false)
    end
end
...
function NPC:setAnimation()
  ...
  if self.animation == "panthersleep" then
    LoadAnimationDic("creatures@cougar@amb@world_cougar_rest@idle_a")
    TaskPlayAnim(self.entity, "creatures@cougar@amb@world_cougar_rest@idle_a", "idle_a", 2.0, 0, -1, 0, 0, 0, 0, 0)
  end
  if self.animation == "catsleep" then
    LoadAnimationDic("creatures@cat@amb@world_cat_sleeping_ground@enter")
    TaskPlayAnim(self.entity, "creatures@cat@amb@world_cat_sleeping_ground@enter", "enter", 2.0, 0, -1, 0, 0, 0, 0, 0)
    SetPedComponentVariation(self.entity, 0, 0, 1, 2)
  end
  if self.animation == "catsleepmeow" then
    LoadAnimationDic("creatures@cat@amb@world_cat_sleeping_ground@enter")
    TaskPlayAnim(self.entity, "creatures@cat@amb@world_cat_sleeping_ground@enter", "enter", 2.0, 0, -1, 0, 0, 0, 0, 0)
    SetPedComponentVariation(self.entity, 0, 0, 2, 2)
  end
end
