...
function Blip:hide(state)
    if DoesBlipExist(self.handle) then
        if state then
            SetBlipAlpha(self.handle, 0)
            SetBlipHiddenOnLegend(self.handle, true)
        else
            SetBlipAlpha(self.handle, 255)
            SetBlipHiddenOnLegend(self.handle, false)
        end
    end
end
