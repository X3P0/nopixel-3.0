...
function GetEntityContext(pEntity, pEntityType, pEntityModel)
    ...
    elseif context.type == 2 then
        context.meta = exports['np-vehicles']:GetVehicleMetadata(pEntity)
        context.flags = exports['np-flags']:GetVehicleFlags(pEntity)
    elseif context.type == 3 then
        context.meta = exports['np-objects']:GetObjectByEntity(pEntity)
...
function hasJobPermission(pJob, pPermission)
  ...
  local hasJobPermission = function()
    if pPermission == "employee" then
      ...
      return RPC.execute('np-business:hasPermission', pJob, pPermission)
    end
  end
...
RegisterNetEvent("np-interact:bustVehicleOwnerShipCache", function(pEntity)
...
function getVehicleOwner(pEntity)
  ...
  local vehicleOwner = NPX.Procedures.execute("np-ottosauto:getVehicleOwner", vin)
...
AddEventHandler("np-fishing:currentlyRentedBoat", function(handle)
    currentlyRentedBoat = handle
end)

AddEventHandler("np-interact:openFridge", function()
  TriggerEvent("server-inventory-open", "31", "Craft");
end)

AddEventHandler("np-interact:grabDrink", function(args, entity, context)
  TriggerEvent("server-inventory-open", "1", context.zones["bar:grabDrink"].name)
end)

RegisterNetEvent('np-interact:setNetVehicleBypassed', function(netId, bypass)
