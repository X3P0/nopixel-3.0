...
AddEventHandler("carryEntity", function(Context, pEntity)
    -- As of what I can see right now the only time this is ever called is to carry biycles but I will assume we later on
    -- want to carry other props as well so added `whitelistedModels` to the list of props that can be carried.
    local entityModel = GetEntityModel(pEntity)
    if (not IsThisModelABicycle(entityModel) and not whitelistedModels[entityModel]) then
        return TriggerEvent("DoLongHudText", "Seems like I cant carry this...", 2)
    end
