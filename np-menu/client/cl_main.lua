MenuTypes, MenuEntries, MenuItems = {'general', 'peds', 'vehicles', 'objects', 'news', 'k9', 'judge', 'meth', 'games'}, {}, {}
...
function IsMenuWanted(pMenu, pEntity)
    return pMenu == 'general' or pMenu == 'peds' and pEntity == 1 or pMenu == 'vehicles' and pEntity == 2 or pMenu == 'objects' and pEntity == 3 or pMenu =='news' and isNews or pMenu == 'k9' and isPolice or pMenu == 'judge' and isJudge or pMenu == 'meth' or pMenu == 'games'
end
    ...
    for index, item in ipairs(pItems) do
        ...
        local success = pcall(function ()
            if entry and (not entry['isEnabled'] or entry['isEnabled'](pEntity, pContext)) then
                ...
                TriggerEvent("i18n:translate", subMenu.title, 'menu:f1:submenu')
                subMenu.title = exports["np-i18n"]:GetStringSwap(subMenu.title)
                ...
                if index % MAX_MENU_ITEMS == 0 and index < (#pItems - 1) then
                    previous[MAX_MENU_ITEMS + 1] = {
                        id = "_more",
                        title = _L("menu-context-more", exports["np-i18n"]:GetStringSwap("More")),
                        icon = "#more",
                        items = current
                    }
                    previous = current
        ...
        if IsMenuWanted(menuType, pContext.type) then
            for _, entry in ipairs(MenuEntries[menuType]) do
                if entry.isEnabled(pEntity, pContext) then
                    ...
                    TriggerEvent("i18n:translate", menu.title, 'menu:f1:menu')
                    menu.title = exports['np-i18n']:GetStringSwap(menu.title);
...
local subMenuPassedCheck = false
function CheckSubMenuItems(pAction, pItems, pEntity, pContext)
    for index, item in ipairs(pItems) do
        local entry = MenuItems[item]
        if entry.data.event == pAction then
            if entry and (not entry.isEnabled or entry.isEnabled(pEntity, pContext)) then
                subMenuPassedCheck = true
                return true
            end
        end
    end
...
function CheckEnabledOnTrigger(pAction, pEntity, pContext)
    for _, menuType in ipairs(MenuTypes) do
        if IsMenuWanted(menuType, pContext.type) then
            for _, entry in ipairs(MenuEntries[menuType]) do
                if entry.data.event == pAction then
                ...
                if entry.subMenus ~= nil and #entry.subMenus > 0 and not subMenuPassedCheck then
                    local isSubmenuItemEnabled = CheckSubMenuItems(pAction, entry.subMenus, pEntity, pContext)
                    if isSubmenuItemEnabled then
                        return true
                    end
                end
            end
...
RegisterNUICallback('triggerAction', function(data, cb)
    ...
    if data.entity and (data.context and data.context.distance) then
        local context = GetEntityContext(data.entity)
        isEnabled = CheckEnabledOnTrigger(data.action, data.entity, context)
    end
