...
function Apart.FindApartmentGivenNumber(roomNumberSent)
    ...
    if roomType == 0 then 
        TriggerEvent("DoLongHudText",_L("apartments-room-error", "Invalid location"),2)
        return false,false
    else
        if roomNumberSent == 0 or roomNumberSent == nil then 
            if roomNumber == 0 then
                TriggerEvent("DoLongHudText",_L("apartments-room-error2", "Invalid input given, add a room number to enter"),2)
                return false,false
            else
